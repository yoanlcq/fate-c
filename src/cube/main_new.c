...
*/
    ...
    m->ctx = SDL_GL_CreateContext(m->window);
    if(!m->ctx)
...
void cube_main_loop_iteration(void *arg) {
    ...
    static vec3 eye = {0.0f, 0.0f, -distance}, center = {0,0,0}, up = {0,1,0};
    static GLint MVPMatrixLoc = glGetUniformLocation(progid, "MVPMatrix");
    static GLint ufInvertLoc  = glGetUniformLocation(progid, "ufInvert");
    static mat4 MVPMatrix, Projection, View, Model;
