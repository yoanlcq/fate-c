...
{
    ++(fgm_shaders_db.top);
    fgm_shaders_db.entries = fe_fatal_realloc(fgm_shaders_db.entries,
            fgm_shaders_db.top,
    ...
    for(i=0 ; i<fgm_shaders_db.top-1 ; ++i) {
        ...
        if(en->hash == fgm_shaders_db.entries[i].hash) {
            fe_logf_err("fe_gl_mkprog: Hash collision detected "
...
static GLenum fgm_shader_type_from_extension(const char *path) {
    ...
    if(!file_extension) {
#ifdef FE_DEBUG_BUILD
        fe_logf_video("\"%s\" has no file extension, "
...
GLuint fgm_find_or_compile_shader(const char *path) 
{
    ...
    if(!file) {
        fe_logf_video("Could not open \"%s\".\n", path);
        return 0;
    }
    GLuint shid = glCreateShader(shtype);
    fe_glObjectLabel(GL_SHADER, shid, -1, path);
    fe_logf_video("Compiling \"%s\"...\n", path); 

    size_t num_strings;
    char **shsrc = fe_file_to_string_array(file, &num_strings);
    fclose(file);
    ...
    fe_logf_video("Could not compile \"%s\" :\n\t", path);
    fe_gl_log_shader_info(shid);
    fe_logf_video("\n");
    glDeleteShader(shid);
    return 0;
}
...
static inline bool fgm_file_is_outdated(const char *path, va_list ap) {
    uint64_t mtime = fe_sys_get_last_write_time(path);
    for(;;) 
    {
        ...
        if(path[0] == '\0')
            continue;
        if(fe_sys_get_last_write_time(path) > mtime)
            return true;
    }
    return false;
}
...
static bool fgm_program_from_binary(GLuint program, FILE *binfile, 
    ...
    uint64_t binlen;
    char *bin = fe_file_to_string(binfile, &binlen);
    ...
    if(err != GL_INVALID_ENUM) {
        ...
        if(status == GL_TRUE) {
            fe_logf_video("Successfully reused \"%s\".\n", save_path);
            return true;
        }
    }
    fe_logf_video("Could not reuse \"%s\" : \n\t", save_path);
    if(err != GL_INVALID_ENUM) {
        fe_gl_log_program_info(program);
        fe_logf_video("\n");
    } else
        fe_logf_video("Either 0x%x is an unrecognized binary format, "
...
static void fgm_program_to_binary(GLuint program, FILE *binfile) {
    ...
    fe_logf_video("Saved binary with format 0x%x.\n", binfmt);
...
static int fe_gl_mkprog_2_0_real(GLuint program, const char *save_path, 
                                   va_list ap)
{
    const char *path;
    int all_good = 1;
    ...
    for(;;) {
        ...
        if(shid)
            ...
            fe_logf_video("Could not compile/reuse \"%s\".\n", path);
            all_good = 0;
        }
    }
    ...
    glGetProgramiv(program, GL_LINK_STATUS, &status);
    if(status == GL_TRUE) {
        fe_logf_video("Successfully linked \"%s\".\n", save_path);
#ifdef FE_GL_DEBUG
        glValidateProgram(program);
        glGetProgramiv(program, GL_VALIDATE_STATUS, &status);
        fe_logf_video("\"%s\" is %svalid. ", 
    ...
    fe_logf_video("Could not link \"%s\" :\n\t", save_path);
    fe_gl_log_program_info(program);
    fe_logf_video("\n");
...
static int fe_gl_mkprog_4_1(GLuint program, const char *save_path, ...) {
    ...
    if(!binfile || binfile_is_outdated) {
        ...
        if(!binfile) {
            fe_logf_video("Could not save program binary to \"%s\".\n",
                            save_path);
            return 1;
        }
...
static int fe_gl_mkprog_2_0(GLuint program, const char *save_path, ...) {
    va_list ap;
    va_start(ap, save_path);
    int retval = fe_gl_mkprog_2_0_real(program, save_path, ap);
    va_end(ap);
    return retval;
}

int (*fe_gl_mkprog)(GLuint program, const char *save_path, ...);
...
void fe_gl_mkprog_setup(GLint gl_major, GLint gl_minor) {
    ...
    if(version < 20) {
#ifdef FE_DEBUG_BUILD
        fe_logf_video("fe_gl_mkprog is not available, because the OpenGL "
