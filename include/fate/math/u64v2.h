...
#ifndef FE_MATH_U64V2_H
#define FE_MATH_U64V2_H
...
#include <math.h>
        ...
        && __has_builtin(__builtin_shufflevector)
            #include <fate/math/vext/u64v2.h>
        #else
...
fe_static_assert(offsetof(fe_u64v2, r) == offsetof(fe_u64v2, at[0]), "");
fe_static_assert(offsetof(fe_u64v2, g) == offsetof(fe_u64v2, at[1]), "");



/*! \brief TODO */
static inline void fe_u64v2_shuffle(fe_u64v2 *r, 
                    const fe_u64v2 *v, 
                    uint64_t m0, uint64_t m1
                    
                    
                    ) {
    fe_u64v2 tmp = *v; /* Aliasing is allowed */
    r->at[0] = tmp.at[m0%2];
    r->at[1] = tmp.at[m1%2];
...
/*! \brief TODO */
static inline void fe_u64v2_shuffle2(fe_u64v2 *r, 
                    const fe_u64v2 *u,
                    const fe_u64v2 *v,
                    uint64_t m0, uint64_t m1
...
/*! \brief TODO */
static inline void fe_u64v2_add(fe_u64v2 * r, const fe_u64v2 * a, const fe_u64v2 * b) {
	size_t i;
	for(i=0; i<2; ++i)
		r->at[i] = a->at[i] + b->at[i];
}
/*! \brief TODO */
static inline void fe_u64v2_sub(fe_u64v2 * r, const fe_u64v2 * a, const fe_u64v2 * b) {
	size_t i;
	for(i=0; i<2; ++i)
		r->at[i] = a->at[i] - b->at[i];
}
/*! \brief TODO */
static inline void fe_u64v2_scale(fe_u64v2 * r, const fe_u64v2 * v, const uint64_t s) {
	size_t i;
	for(i=0; i<2; ++i)
		r->at[i] = v->at[i] * s;
}
