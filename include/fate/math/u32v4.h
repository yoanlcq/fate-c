...
#ifndef FE_MATH_U32V4_H
#define FE_MATH_U32V4_H
...
/*! \brief TODO */
static inline void fe_u32v4_shuffle(fe_u32v4 *r, 
                    const fe_u32v4 *v, 
                    uint32_t m0, uint32_t m1
                    , uint32_t m2
                    , uint32_t m3
                    ) {
    fe_u32v4 tmp = *v; /* Aliasing is allowed */
    r->at[0] = tmp.at[m0%4];
    r->at[1] = tmp.at[m1%4];
    r->at[2] = tmp.at[m2%4];
    r->at[3] = tmp.at[m3%4];
...
/*! \brief TODO */
static inline void fe_u32v4_shuffle2(fe_u32v4 *r, 
                    const fe_u32v4 *u,
                    const fe_u32v4 *v,
                    uint32_t m0, uint32_t m1
                    , uint32_t m2
...
/*! \brief TODO */
static inline void fe_u32v4_add(fe_u32v4 * r, const fe_u32v4 * a, const fe_u32v4 * b) {
	size_t i;
	for(i=0; i<4; ++i)
		r->at[i] = a->at[i] + b->at[i];
}
/*! \brief TODO */
static inline void fe_u32v4_sub(fe_u32v4 * r, const fe_u32v4 * a, const fe_u32v4 * b) {
	size_t i;
	for(i=0; i<4; ++i)
		r->at[i] = a->at[i] - b->at[i];
}
/*! \brief TODO */
static inline void fe_u32v4_scale(fe_u32v4 * r, const fe_u32v4 * v, const uint32_t s) {
	size_t i;
	for(i=0; i<4; ++i)
		r->at[i] = v->at[i] * s;
}
...
/*! \brief TODO */
static inline void fe_u32v4_mul_cross(fe_u32v4 * r, const fe_u32v4 * a, const fe_u32v4 * b) {
